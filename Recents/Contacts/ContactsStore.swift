...
//                }
    ...
    func updateContacts() {
        ...
        do {
            ...
            allContacts = Dictionary(grouping: contacts, by: { $0.firstName?.first! ?? "n"})
            sections = Set(contacts.map { $0.firstName?.first! ?? "n" }).sorted()
        } catch {
    ...
    var firstLetter: [Character] {
        ...
        contacts.forEach { firstLetters.insert($0.firstName?.first! ?? "n") }
